...
static int nrf24l01_worker_do_work(void* ctx)
{
	int err;
	unsigned int data;
	struct nrf24l01_t* nrf = (struct nrf24l01_t*) ctx;
...
//				do_exit(err);
		...
		if(data)
		{
			...
			// Start run Workstruct of RX
			schedule_work(&nrf->rx_workstruct);
		...
		if(data)
		{
			...
			if(data)
			{
				//Notify IEEE subsystem know the previous packet is tranferred success
				nrf24l01_ieee_tx_complete(nrf->spi);
...
int nrf24l01_create_worker(void *Args)
{
	char thread_name[25];
	nrf24l01_struct_t *nrf = (nrf24l01_struct_t *)Args;
...
int nrf24l01_destroy_worker(void *Args)
{
	nrf24l01_struct_t *nrf = (nrf24l01_struct_t *)Args;
