...
static int nrf24l01_probe(struct spi_device* spi)
{
	...
	net_hw = ieee802154_alloc_hw(sizeof(*nrf), &nrf24l01_hw_ops);
	if(!net_hw)
	{
		dev_err(&spi->dev, "%s: failed to allocate memory for network device\r\n", __func__);
		return -ENOMEM;
	}
	...
	if(!nrf)
	{
		dev_err(&spi->dev, "%s: failed to allocate device driver nrf24l01 struct\r\n", __func__);
		return -ENOMEM;
	}
	...
	if(IS_ERR(nrf->regmap_short))
	{
		...
		dev_err(&spi->dev, "%s: Failed to create map register\r\n", __func__);
		goto exit_nrf_list_insert;
	}
	if((err = nrf24l01_create_partregs(nrf)) < 0)
	{
		dev_err(&spi->dev, "%s: Failed to create parts table register\r\n", __func__);
	...
	of_gpio_ce = of_get_property(spi->dev.of_node, "ce-gpio", NULL);
	if(!of_gpio_ce)
	{
	...
	err = nrf24l01_netdeviceregister(net_hw, spi);
	if(err)
	{
...
static const struct of_device_id nrf24l01_of_match[] = {
	{ .compatible = "nordic,nrf24", .data = (void*)nRF24L01 },
	{}
};
...
MODULE_AUTHOR("Tuan Anh refer on Tobias Schramm");
MODULE_LICENSE("GPL");
